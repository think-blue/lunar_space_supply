...
def worker_process(t, worker_model, counter, params, delta_epoch, source, dest, training_results):
    ...
    for i in range(params['epochs']):
        ...
        mlflow.log_metric(key="mean_batch_actor_loss", value=actor_loss.mean().detach().numpy().item(), step=counter.value)
        mlflow.log_metric(key="mean_batch_critic_loss", value=critic_loss.mean().detach().numpy().item(), step=counter.value)
