...
class Metric:
    ...
    def __init__(self):
        self.losses = []
        self.critic_losses = []
        self.actor_episode_loss = {}
        self.critic_episode_loss = {}
    ...
    def cache_loss(self, episode, actor_loss, critic_loss=None):
        self.losses.append(actor_loss)
        episode_loss = self.actor_episode_loss.get(episode, [])
        episode_loss.append(actor_loss)
        self.actor_episode_loss[episode] = episode_loss

        if critic_loss is not None:
            self.critic_losses.append(critic_loss)
            episode_loss = self.critic_episode_loss.get(episode, [])
            episode_loss.append(critic_loss)
            self.critic_episode_loss[episode] = episode_loss
    ...
    def plot_loss_trend(self, root_path):
        for label, collection in [
                ['actor', self.actor_episode_loss],
                ['critic', self.critic_episode_loss]
        ]:
            if len(collection.keys()) > 0:
                self._plot_loss(root_path, collection, label)

    def _plot_loss(self, root_path, collection, label):
        average_losses = [np.mean(loss) for loss in collection.values()]
        plt.plot(average_losses, marker='.')
        plt.xlabel('episodes')
        plt.ylabel('loss')
        plt.savefig(f'{root_path}_{label}_loss_trend.png', bbox_inches='tight')
        plt.close()
        
    def has_converged(self, protein_name):
        config = Config.get_instance()
        run_config = config.run_config
        molecule_metric = self.test_metrices[protein_name]

        scores = molecule_metric[-1].scores
        
        if len(scores) < run_config.window_size:
            return False

        scores = scores[-run_config.window_size:]

        spread = np.std(scores)
        print("SCORE Spread:", spread)

        return spread < run_config.convergence_std
    ...
    def has_diverged(self, episode):
        ...
        return sum(
            molecule_metric[0].rewards
        ) / len(scores) > config.divergence_slope  # percentage 
    ...
    @classmethod
    def exists(cls, root_path):
        return os.path.exists(f'{root_path}_{cls.__SAVE_PATH}')
