...
class LunarEnvironment2(gym.Env, object):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.delta_position = spacecraft_position - target_position
        self.delta_velocity = spacecraft_velocity - target_velocity
        direction_vector, direction_magnitude = self._get_vector(self.delta_position)
        velocity_vector, velocity_magnitude = self._get_vector(self.delta_velocity)
        state = dict(
    ...
    @staticmethod
    def _get_vector(position_velocity):
        delta_x = position_velocity[0]
        delta_y = position_velocity[1]
        delta_z = position_velocity[2]

        magnitude_delta = math.sqrt(delta_x ** 2 + delta_y ** 2 + delta_z ** 2)
