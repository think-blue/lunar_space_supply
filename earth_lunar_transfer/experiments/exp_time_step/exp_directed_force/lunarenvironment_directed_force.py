...
class LunarEnvForceHelper(LunarEnvironment):
    ...
    def _get_reward(self):
        ...
        if np.linalg.norm(self.spacecraft_position - self.source_planet.eph(self.current_epoch)[0]) < 1637e3 + 1000e3:
            moon_region_penalty = -5 / 1e3 * ((1737e3 + 1000e3) - np.linalg.norm(
                self.spacecraft_position - self.source_planet.eph(self.current_epoch)[0]))
        ...
        if np.linalg.norm(
                self.spacecraft_position - self.destination_planet.eph(self.current_epoch)[0]) < 6638e3 + 2000e3:
            earth_region_penalty = -5 / 1e3 * ((6638e3 + 2000e3) - np.linalg.norm(
                self.spacecraft_position - self.destination_planet.eph(self.current_epoch)[0]))
        if np.linalg.norm(
                self.spacecraft_position - self.destination_planet.eph(self.current_epoch)[0]) < 6738e3 + 2000e3:
