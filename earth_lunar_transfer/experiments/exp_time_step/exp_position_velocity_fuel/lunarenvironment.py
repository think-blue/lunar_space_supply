...
class LunarEnvPosition(LunarEnvironment):
    ...
    def _get_reward(self):
        ...
        velocity_reward = - np.linalg.norm(
            self.spacecraft_velocity - self.target_velocity) / self.MOON_SPEED_WRT_EARTH
        time_penalty = 0
        if self.time_step > self.max_time_steps:
            time_penalty = -10

        discounted_reward = 1 * math.pow(self.env_config["discount_factor"], self.time_step)
        reward = discounted_reward + velocity_reward + time_penalty + velocity_reward
        # print(positional_reward, mass_reward, velocity_reward)
        reward_components = [discounted_reward, velocity_reward, time_penalty]
